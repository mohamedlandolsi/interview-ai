...
/**
 * Generate demo analysis for interviews without transcripts
 * This endpoint creates realistic analysis data for demonstration purposes
 */
export async function POST(request: NextRequest) {
  try {
    console.log('🎭 Demo analysis endpoint called')
    ...
    // Get the interview session with template
    ...
    console.log(`🎭 Generating demo analysis for session: ${sessionId}`)
    console.log(`   Position: ${session.position}`)
    console.log(`   Template: ${session.template?.title}`)

    // Generate realistic transcript based on position and template
    const mockTranscript = generateMockTranscript(session.position, session.template?.title || 'General Interview')

    // Use Gemini 2.5 Flash to analyze the mock transcript
    const analysisResult = await generateAnalysisWithGemini(session, mockTranscript)
    ...
    const updatedSession = await prisma.interviewSession.update({
      ...
      data: {
        ...
        duration: session.duration || Math.floor(Math.random() * 30) + 15, // 15-45 minutes
        ...
        vapi_call_id: `demo_${Date.now()}`,
        vapi_cost: Math.random() * 2 + 0.5, // $0.50 - $2.50
        recording_url: `https://demo-recordings.example.com/${sessionId}.mp3`
      }
    })

    console.log(`✅ Demo analysis generated successfully`)
    ...
    return NextResponse.json({
      ...
      session: {
        ...
        analysis_score: updatedSession.analysis_score,
        hiring_recommendation: updatedSession.hiring_recommendation,
        strengths: updatedSession.strengths,
        key_insights: updatedSession.key_insights
      }
    })
